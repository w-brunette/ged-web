...
function TipoConsultar() {
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            try {
                const paged = await service.obterTipos({
    ...
    return (
        <ModuloContainer>
            <ModuloHeader>
                <NavigationBar>
                    <Typography variant="caption">{t(`tipos`)}</Typography>
                    <Button startIcon={<Add />} variant="contained"
                        color="primary"
                        LinkComponent={NavLink} to="../tipo">{t(`incluir`)}</Button>
                </NavigationBar>
            </ModuloHeader>
            <ModuloContent>
                ...
                <Table sx={{ height: 0 }}>
                    ...
                    <TableBody>
                        {tipos && tipos.map(it =>
                            <TableRow key={it.tipoDocumentoId}>
                                <TableCell>{it.codigo}</TableCell>
                                <TableCell>{it.descricao}</TableCell>
                                <TableCell align="right">
                                    <ButtonGroup>
                                        <Button LinkComponent={NavLink} to={`../tipo/${it.tipoDocumentoId}`}>
                                            <ArrowForward />
                                        </Button>
                                    </ButtonGroup>
                                </TableCell>
